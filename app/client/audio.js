SP_APP.audio = {
  ...
  start() {
    ...
    const pitchSamples = {
      ...
      get avg() {
        return this.samples.reduce((result, time) => {
          return result += time;
      ...
      getKey() {
        ...
        const distanceToMatch = Math.min(distanceToLower, distanceToHigher);
        const closestKey = distanceToMatch === distanceToLower
          ? closestLower
          : closestHigher;
      ...
      dataArray.forEach((item, i) => {
        ...
        if (i > 0 && i < dataArray.length && item > 128 && lastItem <= 128) {
          ...
          if (elapsedSteps > STEPS_THRESHOLD) {
            const seconds = elapsedSteps / sampleRate; // sampleRate = 44100
        ...
        const height = didCrossOver ? 50 : 2;
